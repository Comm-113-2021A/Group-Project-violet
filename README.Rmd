...
The goal of our research is to identify the academic and social factors that lead to alcoholism in teenagers and to construct a model to identify the likelihood of teenagers suffering from alcoholism. Our dataset was sourced from highschool students in Portugal, and was found on Kaggle.

We started with exploratory data analysis of some variables we thought strongly contributed to alcohol consumption, creating visualizations. From these visualizations, a few things stood out. Those who have high weekend alcohol consumption (greater than 3), tend to have a total alcohol consumption over three. Alcoholism seems to depend more on weekend alcohol consumption than daily alcohol consumption. Men have a higher average total alcohol consumption than women. As age increases, total alcohol consumption seems to increase. However, this is likely due to a few outlying points with high age and high total alcohol consumption. High schoolers who choose their school based on reputation have the lowest total alcohol consumption while those who choose for reasons besides courses, closeness to home, and reputation, have the highest. High schoolers with parents who live together have a higher average total alcohol consumption value than those whose parents live apart. 

To start modeling and predicting data, we created a linear regression model with the variables we identified after doing exploratory data analysis. There were some issues, however.

The residual plot of the linear regression model was problematic and showed a distinct increase in variability, a fan-shaped pattern. The residual plot of linear regression with all variables as predictors showed the same relationship. Thus, we concluded that a linear model would not be a good predictor. Moreover, the variable of interest was initially set on a rigid scale from 1-5, meaning that a linear regression model which wasn't limited from 1-5 would have issues in interpretation in context of the data. 

Therefore, we decided to use logistic regression, and define a student to be an "alcoholic" if their total alcohol consumption was greater than or equal to 3. We created two logistic regression models. The first with variables we thought significantly contributed to alcohol intake, and the second with variables we did not think contributed. As expected, the first model performed better, but only slightly so. Both models had horrible AUC, log_loss, and ROC graph values. The high log_loss values are particularly concerning, as it indicates that when our model predicts incorrectly it does so by a significant amount. Despite this, both models had high accuracy values, and both usually predicted non-alcoholics correctly. However, the second model predicted everyone to be non-alcoholic. 
...
Looking forward, it would be beneficial to get our hands onto a larger data set with more individuals who were reported as alcoholic, to better train our model. It would also be helpful to extrapolate potential causes of alcoholism, as we define it, to better help individuals.
